...
- `commands/` - 46 slash command definitions, one `.md` per command. **This is the platform-neutral source.** Adapters compile it for each platform.
...
- `scripts/eval/` - retrieval and behavior eval harness (`behavior_eval.py`, `retrieval_eval.py`, `semantic_search.py`, `corpus.py`, `diagnose.py`). Measures whether vault notes are actually retrievable by a future agent. `*.example.jsonl` are the checked-in case fixtures.
...
- `docs/` - **generated** GitHub Pages site, built from `commands/` by `scripts/build_site.py`. Do not hand-edit; regenerate after any command `description:`/`triggers` change (see Testing locally).
- `integrations/obsidian-plugin/` - a TypeScript re-implementation of `references/ai-first-rules.md` as an Obsidian plugin. It is a **second implementation of the spec in a second language** and has its own CI gate (typecheck + `npm run build` + `npm test`). Keep it in sync when the AI-first spec changes.
- `hooks/` - Claude Code hooks shipped with the skill. `validate-ai-first.sh` enforces the AI-first spec (including the banned-character check) on vault writes.
- `SKILL.md` - full operating manual loaded by Claude when the skill activates.
- `architecture.md` - how the layers fit together.
- `README.md` - public-facing docs on github.com. Contains a per-platform conformance board generated by `scripts/conformance_report.py`.
...
There is a smoke-test suite (`tests/test_smoke.py`) that guards the build pipeline (adapter output paths, `vault_health` JSON, OKF export, and similar). CI runs the whole `tests/` tree on every push. Run it locally before pushing any change to `adapters/`, `scripts/`, or `commands/`:
...
`.github/workflows/ci.yml` runs six checks. Reproduce them locally before pushing:

```bash
ruff check .                                  # gated rule set: F, I, PTH (see pyproject.toml)
uv run pytest -q                              # full tests/ tree
python scripts/sweep_non_ascii.py --check     # banned substitution chars in tracked prose
python scripts/conformance_report.py --check  # README conformance board matches a real build
python scripts/build_site.py --check          # docs/ site matches commands/
```

The sixth gate builds and tests the Obsidian plugin:

```bash
cd integrations/obsidian-plugin && npm ci && npm run build && npm test
```

When you change a command's `description:` or `triggers`, regenerate both generated artifacts in the same commit:

```bash
uv run python scripts/build_site.py           # rewrites docs/
uv run python scripts/conformance_report.py --write   # rewrites the README board
```
