...
**Two optional fences on a mounted connection (MCP only).** A client cannot switch off single tools, so a profile that should only touch part of the vault is scoped with env vars on the server. Both take colon-separated, vault-relative folder prefixes and both match a whole path component, case-sensitively.

- **Write fence** - `OBSIDIAN_MCP_WRITE_ALLOW` names the folders this connection may write (`save_note`, `capture`, `update_note`, `replace_text`, `move_note`). Unset means unrestricted; empty means read-only. A refused write returns an error naming the allowed prefixes and changes nothing.
- **Read fence** - `OBSIDIAN_MCP_READ_ALLOW` names the folders this connection may read. It applies to every read path: `obsidian_search` (out-of-scope notes are dropped in the walk, before the scan cap and the result limit, and the semantic arm is filtered too, so nothing can be inferred from result counts), `obsidian_read_note` (a clear error naming the readable scope, never an empty note), `obsidian_backlinks`, `obsidian_vault_health` and `obsidian_validate_note`. **Unset means allow everything, exactly the behaviour with no fence at all**, so nothing changes until a profile sets it; empty means this connection may read no notes. `obsidian_search` also takes `folders=["Specs/", "Decisions/"]` to narrow one call, intersected with the fence so it can never widen it.

The two are independent and compose (read `Decisions/`, write only `Knowledge/`). Neither replaces filesystem permissions: a client that also holds direct file tools on the vault path can read around both. Which folders each role may read is decided in Capsule Corp's C4 tick table, not in this repo. Details in `integrations/obsidian-mcp-server/README.md`, "Scoping a mounted connection: two fences".
